...
from wanx.models.activity import ActivityConfig, ActivityVideo, VoteVideo, Mteam, VoteMteam, \
    MatchCourse, TeamSupport, TeamSupportRecord, Battle
...
@video.route('/team_support', methods=['GET'])
...
def activity_team_support():
    ...
    # 增加ip限制，每个ip每天限制20次
    user_ip = request.remote_addr
    ip_key = 'ip_limit:%s:%s:%s:%s' % (user_ip, battle_id, team_name, str(datetime.date.today()))
    ip_limit = Redis.incr(ip_key)
    if ip_limit == 1:  # 第一次设置key过期时间
        Redis.expire(ip_key, 86400)
    ...
    team_support_record = TeamSupportRecord.get_team_support_record(device, battle_id, team_name)
