...
from wanx.base import util, error, const, spam
from wanx.base.xredis import Redis
from wanx.models.game import VipGiftCode
from wanx.platforms.migu import Marketing, MiguPay
from wanx.models.store import Store, StoreItem, UserOrder, UserOrderAddress, BarrageColor, UserGiftCodeOrder
...
def lottery_info():
    ...
    if user:
        ...
        if isinstance(chances, error.ApiError):
            return chances
        if int(chances) == 0:
            ors = UserOrder.get_user_store_orders(user._id, store_id, 1, store.lotterynum)
            chances = store.lotterynum - len(ors)
        if store.yxmember:
            uservip = MiguPay.check_user_vip_level(user.phone)
            if isinstance(uservip, error.ApiError):
                return uservip
            if not (uservip['vip5']['subscribed'] or uservip['vip10']['subscribed']):
                chances = -1
    ...
    if left_chances <= 0:
        ...
        with util.Lockit(Redis, key) as locked:
            ...
            if not trigger:
                ...
                ret = Marketing.execute_campaign(user.partner_migu['id'], user.phone,
                                                 [store.campaign_id], trigger=trigger)
...
@app.route('/store/barrage_color', methods=['GET', 'POST'])
@util.jsonapi()
def barrage_color():
    """获取弹幕颜色列表(GET)
    ...
    :return: {'stores': <Store>list}
    """
    colors = BarrageColor.get_colors()
    colorlist = [color.format() for color in colors]
    return {'colors': colorlist}


@app.route('/store/<int:event_id>/add_barrage_color', methods=['GET', 'POST'])
@util.jsonapi(login_required=True)
