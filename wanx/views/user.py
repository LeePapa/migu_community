...
from wanx.models.user import User, FriendShip, UserShare, UserCertify
...
def platform_login():
    ...
    if platform == 'migu':
        ...
        if not user:
            ...
            if user:
                info = dict(
                    partner_migu={'id': migu_uid, 'passid': passid},
                    name='$mg$%s%s' % (migu_uid[-4:], random.randint(1000, 9999))
                )
                user = user.update_model({'$set': info})
            else:
            ...
            if passid not in user.partner_migu:
                info.update(dict(partner_migu={'id': migu_uid, 'passid': passid}))
            if info:
                user = user.update_model({'$set': info})
...
@app.route("/users/certify", methods=("POST", "GET",))
@util.jsonapi(login_required=True)
def certify():
    """实名认证 (GET|POST)
...
@app.route("/users/certify_status", methods=("POST", "GET",))
@util.jsonapi(login_required=True)
def certify_status():
    """获取实名认证状态 (GET|POST)
...
@app.route("/users/certify_relieve", methods=("POST", "GET",))
@util.jsonapi(login_required=True)
def certify_relieve():
    """解除实名认证 (GET|POST)
    ...
    Redis.delete(UserCertify.OBJECT_KEY % ({'name': 'users', 'oid': str(user_id)}))
    ...
    if UserCertify.is_mobile(phone):
        con = '【咪咕游戏】您好，您的账号{0}已与身份证号{1}成功解绑，请确保是您本人操作。'.format(phone, ID_number)
...
@app.route("/users/certify_sms_code", methods=("POST", "GET",))
@util.jsonapi(login_required=True)
...
@app.route('/users/reminding', methods=['GET', 'POST'])
@util.jsonapi(login_required=True)
def reminding():
    """获取用户的提醒 (GET)
    ...
    while len(users) < pagesize:
        ...
        map(lambda u:u.update({'isreminded': True}) if u['user_id'] in reminding_ids else u.update({'isreminded': False}), user_followings)
...
@app.route('/users/changeremind', methods=['GET', 'POST'])
@util.jsonapi(login_required=True)
def changeremind():
    """修改用户关注提醒状态
    ...
    :uri: /users/contacts
    :param reminder: 用户关注
    :param status: 提醒状态（默认为0）
    :returns: {'users': list}
    """
    user = request.authed_user
    params = request.values
    reminder = params.get('reminder', None)
    status = params.get('status', None)
    friend_ship = FriendShip.get_by_ship(user._id, reminder)
    if not reminder or not friend_ship:
        return error.InvalidArguments
    friend_ship.update_remind(status)
    return {}


@app.route('/user/remindnums', methods=['GET', 'POST'])
@util.jsonapi(login_required=True)
def getallreminding():
    """
    ...
    user_followings = [{"phone": u.phone, "user_id": str(u._id)} for u in User.get_list(uids)]
    reminding_ids = FriendShip.reminding_ids(user._id)
    map(lambda u: u.update({'isreminded': True}) if u['user_id'] in reminding_ids else u.update({'isreminded': False}),
        user_followings)
...
@app.route('/users/live/history', methods=['GET'])
@util.jsonapi(login_required=True)
def user_live_history():
    """
    用户直播时长历史记录
    :return: {'logs': logs}
    """
    user = request.authed_user
    uid = str(user._id)
    logs = Event.get_user_live_history(uid)
    return {'logs': logs, 'duration': sum([i['duration'] for i in logs])}
