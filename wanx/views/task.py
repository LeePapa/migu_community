...
def user_tasks():
    ...
    daily_tasks = sorted(filter(platform_filter, daily_tasks), key=lambda x: (x['order'], x['create_at']))
    novice_tasks = [utask.format() for utask in UserTask.get_novice_tasks(str(user._id))]
    novice_tasks = sorted(filter(platform_filter, novice_tasks), key=lambda x: (x['order'], x['create_at']))
    game_tasks = [utask.format() for utask in UserTask.get_game_tasks(str(user._id))]
    game_tasks = sorted(filter(platform_filter, game_tasks), key=lambda x: (x['order'], x['create_at']))
    activity_tasks = [utask.format() for utask in UserTask.get_activity_tasks(str(user._id))]
    activity_tasks = sorted(filter(platform_filter, activity_tasks), key=lambda x: (x['order'], x['create_at']))
