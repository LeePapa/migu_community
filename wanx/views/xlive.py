...
from wanx.models.live import WatchLiveTask, WatchLiveTaskItem, LiveRedPacket, UserRedPacket, \
    LiveRedPacketItem, \
    HotWords
...
def get_live_info():
    ...
    for rp in LiveRedPacket.get_list(rp_ids):
        ...
        if rp.login == 'login' and (not uid or not rp.user_in_group(str(rp.group), uid)):
            continue
    ...
    battle = Battle.get_live_battle(live['user_id'], live['name'], device)
    live['from_following'] = False
    if user:
        uids = FriendShip.following_ids(str(user._id))
        if live['user_id'] in uids:
            live['from_following'] = True
    ex_fields = ['game__subscribed']
    return {'live': Xlive.format(live, exclude_fields=ex_fields),
            'task': task, 'red_packet': red_packet, 'cdrp': cdrp, 'battle': battle}
...
def grab_redpacket():
    ...
    with util.Lockit(Redis, key) as locked:
        ...
        # 从分享红包获取物品
        prize = Marketing.grab_red_package(user.partner_migu['id'], red_packet.campaign_id,
...
@app.route('/lives/<int:event_id>/userjoin', methods=['POST', 'GET'])
@util.jsonapi(login_required=True)
def user_livejoin(event_id):
    """用户进入直播间推送弹幕
    ...
    data = dict(
        event_id=event_id,
        message=user.nickname or user.name
    )
    Xlive.send_live_msg(data, 'userjoin')
    return {}


@app.route('/live/hot_words', methods=['GET'])
@util.jsonapi()
def live_hot_words():
    hids = HotWords.all_hot_words()

    hot_words = list()
    [hot_words.append(h.hot_words) for h in HotWords.get_list(hids)]

    return {'hot_words': hot_words}


@app.route('/live/battle', methods=['GET'])
@util.jsonapi()
def battle():
    video_id = '585cc7be43bd892c802c9ed5'

    data = {"duration": 581, "user_id": "5823b39763057f65ec280c10",
            "url": "http://112.4.19.166/vod/201701/5/11230_142754.mp4",
            "title": "\u6770\u745e\u59d0\u59d0\u7684\u76f4\u64ad\u95f4", "event_id": "11230",
            "game_id": "56834a64a15e402bfac91d10",
            "cover": "http://112.4.19.166/images/default.png", "ratio": "864x480"}

    b = Battle.get_live_battle(data['user_id'],
                                    data['title'].decode('unicode-escape').encode('utf-8'), None)
    if b:
        Battle.set_video_id(b['_id'], video_id)
