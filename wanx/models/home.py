...
from wanx.models.show import UserSubShow
...
class TitleColor(Document):
    """标题栏颜色
    """
    collection = DB.title_color

    TITLE_COLOR = 'banners:title_color'
    ...
    @classmethod
    @util.cached_list(lambda cls: cls.TITLE_COLOR, snowslide=True)
    def _load_banner_title_color(cls):
        title_color = cls.collection.find({'expire_at': {'$gte': time.time()}, 'status': 0},
                                          {'_id': 1})
        tids = [str(t['_id']) for t in title_color] if title_color else []
        return tids
...
class BottomColor(Document):
    """底部导航栏颜色
    """
    collection = DB.bottom_color

    BOTTOM_COLOR = 'banners:bottom_color'
    ...
    @classmethod
    @util.cached_list(lambda cls: cls.BOTTOM_COLOR, snowslide=True)
    def _load_banner_bottom_color(cls):
        bottom_color = cls.collection.find({'expire_at': {'$gte': time.time()}, 'status': 0},
                                           {'_id': 1})
        bids = [str(b['_id']) for b in bottom_color] if bottom_color else []
        return bids

    @classmethod
    def get_banner_bottom_color(cls):
        key = cls.BOTTOM_COLOR
        if not Redis.exists(key):
            cls._load_banner_bottom_color()
        try:
            bids = Redis.lrange(key, 0, -1)
        except exceptions.ResponseError:
            bids = []
...
class BottomPhoto(Document):
    """底部导航栏图片
    """
    collection = DB.bottom_photo

    BOTTOM_PHOTO = 'banners:bottom_photo'

    def format(self):
        data = {
            'initial_image': urljoin(app.config.get("MEDIA_URL"), self.initial_image),
            'click_image': urljoin(app.config.get("MEDIA_URL"), self.click_image),
            'begin_at': self.begin_at,
            'expire_at': self.expire_at,
            'label': self.label,
        }
    ...
    @classmethod
    @util.cached_list(lambda cls: cls.BOTTOM_PHOTO, snowslide=True)
    def _load_banner_bottom_photo(cls):
        bottom_photo = cls.collection.find({'expire_at': {'$gte': time.time()}, 'status': 0},
                                           {'_id': 1})
        bids = [str(b['_id']) for b in bottom_photo] if bottom_photo else []
        return bids
...
class GameRecommendConfigAdmin(Document):
    """
        游戏推荐模块配置
    """
    collection = DB.game_recommend_config

    ALL_MODULE_GAME_IDS = 'recommend_module_game:{module_id}'  # 模块下的所有游戏

    @classmethod
    @util.cached_list(lambda cls, module_id: cls.ALL_MODULE_GAME_IDS.format(module_id=module_id),
                      snowslide=True)
    def _load_all_module_game_ids(cls, module_id):
        games = list(cls.collection.find(
            {'module_id': ObjectId(module_id)},
            {'game': 1}
        ).sort("order", pymongo.ASCENDING))

        gids = [str(g['game']) for g in games]
        return gids
...
class GameRecommend(Document):
    """
        游戏推荐模块
    """
    collection = DB.game_recommend_module

    ALL_MODULE_IDS = 'recommend_module:all'  # 所有推荐模块
    ...
    @classmethod
    @util.cached_list(lambda cls: cls.ALL_MODULE_IDS, snowslide=True)
    def _load_all_module_ids(cls):
        modules = list(cls.collection.find(
            {'expire_at': {'$gte': time.time()}},
            {'_id': 1}
        ))
        mids = [str(m['_id']) for m in modules]
        return mids
...
class BugReport(BaseModel):
    ...
    def add_report(cls, argvs):
        ...
        data = dict(err_type=err_type, exception=exception, phone_model=phone_model,
...
class UserSubObj:
    USER_SUB_IDS = 'obj:usub:%(uid)s'  # 用户订阅的对象列表
