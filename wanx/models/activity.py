...
class ActivityVideo(Document):
    ...
    def get_activity_video_by_authors(cls, authors, activity_id):
        activity_videos = list(
            cls.collection.find({'author': {'$in': authors}, 'activity_id': ObjectId(activity_id)}))
...
class MatchCourse(Document):
    """
    赛程
    """
    collection = DB.match_course

    @classmethod
    def get_course_by_mname(cls, mname):
        course = cls.collection.find_one({'match_id': ObjectId(mname), 'course_id': '2'})
        return cls(course) if course else None
...
class Battle(Document):
    ...
    @classmethod
    def get_one_data(cls, battle_id):
        return cls.collection.find_one({'_id': ObjectId(battle_id)})

    @classmethod
    def set_video_id(cls, battle_id, video_id):
        ret = cls.collection.update({'_id': ObjectId(battle_id)},
                                    {'$set': {'replay_video_id': ObjectId(video_id)}})

        return ret


class TeamSupport(Document):
    """
        站队支持
    """
    collection = DB.team_support

    @classmethod
    def get_team_support(cls, battle_id, team_name):
        team_support = cls.collection.find_one(
            {'battle_id': ObjectId(battle_id), 'team_name': team_name})

        return cls(team_support) if team_support else None


class TeamSupportRecord(Document):
    """
        站队支持记录
    """
    collection = DB.team_support_record

    def create_model(self):
        _id = super(TeamSupportRecord, self).create_model()
        if _id:
            team_support = TeamSupport.get_team_support(str(self.battle_id), str(self.team_name))
            team_support.update_model({'$inc': {'ticket': 1}})
        return _id

    def update_model(self, data={}):
        obj = super(TeamSupportRecord, self).update_model()
        return obj

    def delete_model(self):
        super(TeamSupportRecord, self).delete_model()
        team_support = TeamSupport.get_team_support(str(self.battle_id), str(self.team_name))
        if team_support:
            team_support.update_model({'$inc': {'ticket': -1}})
        return
    ...
    @classmethod
    def get_team_support_record(cls, device, battle_id, team_name):
        team_support_record = cls.collection.find_one(
            {'device': device, 'battle_id': ObjectId(battle_id), 'team_name': team_name})

        return cls(team_support_record) if team_support_record else None
...
class ActivityLiveMaster(Document):
    ...
    @classmethod
    @util.cached_zset(lambda cls, aid, begin_at, end_at: cls.DUR_MASTERS % ({'aid': aid}),
                      timeout=300, snowslide=True)
    def _load_duration_uids(cls, aid, begin_at, end_at):
        activity = ActivityConfig.get_one(aid)
        user_durations = list()
        umap = {}
        uids = list(cls.collection.find({'activity_id': aid},
                                        {'user_id': 1, 'create_at': 1}))
        map(lambda x: umap.update({
            x['user_id']: {
        ...
        query = {
            'user_id': {'$in': umap.keys()},
            'create_at': {'$gt': begin_at},
            'finish_at': {'$exists': True, '$lt': end_at}
        }
        if activity and activity.game:
            query.update({'game_id': activity.game})
        lives = LIVE_DB.event.find(query)
        for live in lives:
            uid = live['user_id']
            if umap[uid]['create_at'] > live['create_at']:
                continue
            umap[uid]['dur'] = umap[uid]['dur'] + live['finish_at'] - live['create_at']

        for uid, val in umap.iteritems():
            cls.collection.update({'activity_id': aid, 'user_id': uid},
                                  {'$set': {'duration': val['dur']}})
            user_durations.extend([val['dur'], val['_id']])
        return tuple(user_durations)

    @classmethod
    def duration_uids(cls, aid, begin_at, end_at, page, pagesize, maxs=None):
        key = cls.DUR_MASTERS % ({'aid': aid})
        if not Redis.exists(key):
            cls._load_duration_uids(aid, begin_at, end_at)
        try:
            if maxs:
                ids = Redis.zrevrangebyscore(key, '(%.6f' % (maxs), '-inf', start=0, num=pagesize)
            else:
    ...
    def create_model(self):
        ret = super(ActivityLiveMaster, self).create_model()
        if ret:
            self.clear_redis(self.activity_id)
        return ret
...
class ShareActivityConfig(Document):
    ...
    REWARD_COUNTER = 'activity:counter:%(aid)s'
    ...
    def incr_counter(self):
        key = self.REWARD_COUNTER % {'aid': self._id}
...
class AdShareCfg(Document):
    """
    分享页广告配置
    """
    collection = DB.ad_share_cfg

    ALL_AD_BANNER_IDS = 'ad_banners:all'  # 所有广告banner列表
    ...
    @property
    def online(self):
        _is_online = super(AdShareCfg, self).online
        if not _is_online:
            return False
    ...
    @classmethod
    @util.cached_list(lambda cls: cls.ALL_AD_BANNER_IDS, snowslide=True)
    def _load_all_ad_banner_ids(cls):
    ...
    @classmethod
    def all_ad_banner_ids(cls):
        key = cls.ALL_AD_BANNER_IDS
        if not Redis.exists(key):
            cls._load_all_ad_banner_ids()
        try:
