...
class JpushCollect(Document):
    fields = ['jpush_registration_id','imei','phone','uid','create_at','jpush_message_id','_id','my_message_id']

    @classmethod
    def init(cls):
        doc = super(JpushCollect, cls).init()
        return doc

    def save(self,**argw):
        data = {item:value for item,value in argw.items() if item in self.fields}
        data['_id'] = data['jpush_registration_id']+':'+data['my_message_id']
        data['create_at'] = time.time()
        self.save_model(data)

class GiftNum(Document):
    collection = DB.gift_num