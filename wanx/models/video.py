...
class Video(Document):
    ...
    GAME_NOLIVE_VIDEO_IDS = "videos:nolivegame:%(date)s:%(gid)s"  # 非直播回放人气视频队列
    GAME_USERLIVE_VIDEO_IDS = "videos:userlivegame:%(date)s:%(uid)s:%(gid)s"  # 用户直播回放人气视频队列
    ...
    SHOW_CHANNEL_VIDEO_IDS = "videos:show:channel:%(cid)s"  # 某个栏目频道视频列表
    ...
    def format(self, exclude_fields=[]):
        ...
        # 视频栏目信息
        from wanx.models.show import Show, ShowChannel
        channel = ShowChannel.get_one(self.channel)
        show = channel and Show.get_one(channel.show)
    ...
    def search(cls, keyword):
        ids = cls.collection.find(
            ...
            {'_id': 1}
        ).sort("create_at", pymongo.DESCENDING).limit(100)
    ...
    @classmethod
    @util.cached_list(
        lambda cls, gid: cls.GAME_NOLIVE_VIDEO_IDS % ({'date': time.strftime("%Y%m%d"),
                                                       'gid': gid}),
        timeout=86400, snowslide=True)
    def _load_game_nolive_video_ids(cls, gid):
        videos = list(cls.collection.find(
            {
                'game': ObjectId(gid),
                'duration': {'$gte': const.DURATION},
    ...
    @classmethod
    def game_nolive_video_ids(cls, gid, page, pagesize, maxs=None):
        key = cls.GAME_NOLIVE_VIDEO_IDS % ({'date': time.strftime("%Y%m%d"), 'gid': gid})
        if not Redis.exists(key):
            cls._load_game_nolive_video_ids(gid)
        try:
    ...
    @classmethod
    @util.cached_list(
        lambda cls, uid, gid: cls.GAME_USERLIVE_VIDEO_IDS % (
                {'date': time.strftime("%Y%m%d"), 'uid': uid, 'gid': gid}),
        timeout=86400, snowslide=True)
    def _load_game_userlive_video_ids(cls, uid, gid):
        videos = list(cls.collection.find(
            {
                'game': ObjectId(gid),
                'duration': {'$gte': const.DURATION},
    ...
    @classmethod
    def game_userlive_video_ids(cls, uid, gid, page, pagesize, maxs=None):
        key = cls.GAME_USERLIVE_VIDEO_IDS % ({'date': time.strftime("%Y%m%d"), 'uid': uid, 'gid': gid})
        if not Redis.exists(key):
            cls._load_game_userlive_video_ids(uid, gid)
        try:
            start = (page - 1) * pagesize if page else 0
            stop = (start + pagesize - 1) if pagesize else -1
            ids = Redis.lrange(key, start, stop)
    ...
    @classmethod
    def get_incr_videos(cls, maxs, nbr):
        videos = list(cls.collection.find(
            {
                '$or': [
                    {'create_at': {'$gt': maxs}},
        ...
        ).sort([('update_at', 1), ('create_at', 1)]).limit(nbr))
        if videos:
            maxs = max(videos[-1].get('create_at', 0), videos[-1].get('update_at', 0))
        return maxs, [cls(v).format() for v in videos]

    @classmethod
    @util.cached_list(lambda cls, cid: cls.SHOW_CHANNEL_VIDEO_IDS % {'cid': cid}, snowslide=True)
    def _load_show_channel_videos(cls, cid):
        videos = list(cls.collection.find(
            {'channel': ObjectId(cid)},
            {'_id': 1}
        ).sort([
    ...
    @classmethod
    def show_channel_ids(cls, cid, page, pagesize):
        key = cls.SHOW_CHANNEL_VIDEO_IDS % {'cid': cid}
        if not Redis.exists(key):
            cls._load_show_channel_videos(cid)
        try:
            start = (page - 1) * pagesize
            stop = start + pagesize - 1
            ids = Redis.lrange(key, start, stop)
...
class ReportVideo(Document):
    ...
    def format(self):
        ...
        uid = self.uid and str(self.uid)
        source = self.source or 0
        if source == 0:
            obj = Video.get_one(str(self.vid), check_online=False)
            if not obj:
                author = User.get_one(uid, check_online=False)
                title = u'视频不存在或已被删除'
            else:
                author = User.get_one(str(obj.author), check_online=False)
                title = obj.title
        elif source == 1:
            obj = Xlive.get_live(str(self.vid))
            if not obj:
                author = User.get_one(uid, check_online=False)
                title = u'直播间不存在或已关闭'
            else:
                author = User.get_one(str(obj['user_id']), check_online=False)
                title = obj['name']
        elif source == 2:
            obj = Comment.get_one(str(self.vid), check_online=False)
            if not obj:
                author = User.get_one(uid, check_online=False)
                title = u'评论不存在或已被删除'
            else:
            ...
            obj = Reply.get_one(str(self.vid), check_online=False)
            if not obj:
                author = User.get_one(uid, check_online=False)
                title = u'评论回复不存在或已被删除'
            else:
                author = User.get_one(str(obj.owner), check_online=False)
                title = obj.content

        data = {
            'video_id': self.vid,
            'video_title': title,
            'video_author': author and author.nickname,
            'video_author_id': author and str(author._id),
            'type': self.type,
    ...
    @classmethod
    def check_reported(cls, vid, source, from_user):
        return cls.collection.count({'vid': vid, 'source': source, 'from_user': from_user})

    @classmethod
    def get_reported_videos(cls, page, pagesize):
    ...
    def get_list_by_vids(cls, vids):
        ...
        for vid in vids:
            ...
            uid = report.get('uid')
            uid = uid and str(uid)
            source = report.get('source', 0)
            if source == 0:
                obj = Video.get_one(str(vid), check_online=False)
                if not obj:
                    author = User.get_one(uid, check_online=False)
                    title = u'视频不存在或已被删除'
                else:
                ...
                obj = Comment.get_one(str(vid), check_online=False)
                if not obj:
                    author = User.get_one(uid, check_online=False)
                    title = u'评论不存在或已被删除'
                else:
                ...
                obj = Reply.get_one(str(vid), check_online=False)
                if not obj:
                    author = User.get_one(uid, check_online=False)
                    title = u'评论回复不存在或已被删除'
                else:
    ...
    @classmethod
    def reach_max_limit(cls, vid, max_limit):
        return cls.collection.count({'vid': vid}) == max_limit
...
class EditorVideo(Document):
    """
    小编推荐视频
    status: 显示状态
    order: 显示顺序
    """
    collection = DB.editor_video

    ALL_EDITOR_VIDEO_IDS = 'videos:editor_video'

    @classmethod
    @util.cached_list(lambda cls: cls.ALL_EDITOR_VIDEO_IDS, snowslide=True)
    def _load_all_editor_video_ids(cls):
        videos = list(cls.collection.find({}, {'video': 1}).sort("order", pymongo.ASCENDING))
        _ids = [str(b['video']) for b in videos]
        return _ids

    @classmethod
    def all_editor_video_ids(cls):
        key = cls.ALL_EDITOR_VIDEO_IDS
        if not Redis.exists(key):
            cls._load_all_editor_video_ids()
        try:
            _ids = Redis.lrange(key, 0, -1)
        except exceptions.ResponseError:
            _ids = []
