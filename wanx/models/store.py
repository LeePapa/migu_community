...
BARRAGE_COLOR_KEY = 'barrage:color'
USER_GIFT_CODE_KEY = 'giftcodes:user:%(uid)s'
...
BARRAGE_COLOR_RGB = (
    (const.RED, '#EF2C2C'),     # 红色
    (const.YELLOW, '#F2B000'),  # 黄色
    (const.BLUE, '#1C73EB'),    # 蓝色
    (const.GREEN, '#26C42A'),   # 绿色
    (const.PINK, '#FF5994'),    # 粉红色
)
...
class UserOrder(BaseModel):
    ...
    user_ip = pw.CharField(default=None, max_length=16, verbose_name='用户抽奖ip地址')
...
class BarrageColor(BaseModel):
    color_id = pw.PrimaryKeyField(choices=BARRAGE_COLOR, verbose_name='颜色ID')
    credit_type = pw.IntegerField(choices=CREDIT_TYPE, verbose_name='价格类型')
    credit_value = pw.IntegerField(default=0, verbose_name='价格数值')

    class Meta:
        db_table = 'barrage_color'
    ...
    @classmethod
    @util.cached_object(BARRAGE_COLOR_KEY)
    def _load_colors(cls):
        colors = list(cls.select())
        color_list = [model_to_dict(color) for color in colors]
        return color_list
    ...
    def get_color(cls, color_id):
        colors = filter(lambda x: x.color_id == color_id, cls.get_colors())
        return colors[0] if colors else None

    def save(self, *args, **kwargs):
        if self.select().where(BarrageColor.color_id == self.color_id):
            ret = super(BarrageColor, self).save(*args, **kwargs)
        else:
            ret = super(BarrageColor, self).save(force_insert=True, *args, **kwargs)
        return ret
...
class UserGiftCodeOrder(BaseModel):
    ...
    campaign_id = pw.CharField(default='', max_length=64, verbose_name='营销平台活动ID')
    gift_code = pw.CharField(max_length=64, verbose_name='礼包兑换码')
    result = pw.CharField(default='', max_length=1024, verbose_name='营销平台结果')
    recid = pw.CharField(null=True, max_length=128, verbose_name='营销平台订单号')
    expire_at = pw.DateTimeField(constraints=[pw.SQL('DEFAULT CURRENT_TIMESTAMP')],
                                 formats='%Y-%m-%d %H:%M:%S', verbose_name='失效时间')
    status = pw.IntegerField(choices=STATUS, verbose_name='状态')
    user_ip = pw.CharField(default=None, max_length=16, verbose_name='用户抽奖ip地址')
    ...
    @classmethod
    @util.cached_list(lambda cls, uid: USER_GIFT_CODE_KEY % {'uid': uid}, snowslide=True)
    def _load_user_gift_code_ids(cls, uid):
        orders = list(cls.select(UserGiftCodeOrder.vgc_id).where(cls.user_id == uid))
