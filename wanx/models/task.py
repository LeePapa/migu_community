...
class Task(BaseModel):
    ...
    cover = pw.CharField(max_length=255, verbose_name='封面图片')
    ...
    button_action = pw.CharField(max_length=255, verbose_name='按钮跳转链接')
    order = pw.IntegerField(default=1, verbose_name='显示顺序')
    topest = pw.BooleanField(default=0, verbose_name='是否置顶')
    ...
    def format(self):
        ...
        data['cover'] = self.cover and urljoin(app.config.get("MEDIA_URL"), self.cover)
        return data

    @classmethod
    def reset_topest_task(cls):
        cls.update(topest=0).execute()
...
class UserTask(BaseModel):
    ...
    def format(self):
        ...
        if self.task_status == FINISHED:
            ...
            data['btn_desc'] = '已领取' if task.action != DAILY_SIGNIN else '已签到'
        else:
