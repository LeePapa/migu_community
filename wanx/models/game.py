...
class Game(Document):
    ...
    def format(self, exclude_fields=[]):
        ...
        gid = str(self._id)
        covers = [urljoin(app.config.get("MEDIA_URL"), cvr) for cvr in self.covers] if self.covers else []
        data = {
            'game_id': gid,
            'name': self.name,
            ...
            'is_online': self.status in [None, const.ONLINE, const.OFFSHELF],
        ...
        # 如果产品库有对游戏进行手机型号区分，则根据手机型号获取版本信息和软件包名称
        if self.package_id_map:
            ua = request.headers.get('User-Agent')
            platform = request.values.get('os')
            version_code = request.values.get('version_code', 0)

            # Android版本号64以上的版本，需要校验ua获取软件包信息
            if ua and platform == 'android' and int(version_code) >= 64:
                data['package_id'] = self.package_id_map.get(ua, {}).get('package_id', self.package_id)
                data['package_version'] = self.package_id_map.get(ua, {}).get('version', self.version)
                data['package_size'] = self.package_id_map.get(ua, {}).get('size', self.size)
    ...
    @classmethod
    def search(cls, keyword, ua_filter=None):
        if ua_filter:
            key = 'package_id_map.%s' % ua_filter
            ids = cls.collection.find(
                {'name': {'$regex': keyword, '$options': 'i'},
                 'status': {'$nin': [const.OFFLINE, const.UNDER_TEST]},
                 '$or': [{key: {'$exists': True}}, {'status': const.ONLINE}]},
                {'_id': 1}
    ...
    def get_by_content_id(cls, content_id):
        if not content_id:
            return None
        game = cls.collection.find_one({"bcode": content_id})
        return cls(game) if game else None
...
class TopList(Document):
    """游戏排行"""
    collection = DB.top_game_lists
    ALL_IDS = 'games:toplists:%(os)s'
    ...
    def format(self):
        data = {
            ...
            'banner': urljoin(app.config.get("MEDIA_URL"), self.banner),
            'action': self.action,
        }
        return data
...
class TopListGames(Document):
    """排行游戏"""
    collection = DB.top_list_games
    ALL_IDS = 'games:toplist:%(tpid)s'    # 某个模块下的游戏列表

    @classmethod
    @util.cached_list(lambda cls, mid: cls.ALL_IDS % ({'tpid': mid}), snowslide=True)
    def _load_toplist_game_ids(cls, mid):
        games = list(cls.collection.find(
            {'toplist': ObjectId(mid)},
            {'game': 1}
        ).sort("order", pymongo.ASCENDING))
        gids = [str(g['game']) for g in games if Game.get_one(str(g['game']))]
        return gids
    ...
    @classmethod
    def toplist_game_ids(cls, tpid):
        key = cls.ALL_IDS % ({'tpid': tpid})
        if not Redis.exists(key):
            cls._load_toplist_game_ids(tpid)
        try:
            gids = Redis.lrange(key, 0, -1)
        except exceptions.ResponseError:
            gids = []
...
class ChessConfig(Document):
    """棋牌配置"""
    collection = DB.chess_config
    ALL_IDS = 'games:chessconfig:%(os)s'
...
class ChessGames(Document):
    """棋牌游戏"""
    collection = DB.chess_games
    ALL_IDS = 'games:chess:%(cid)s'    # 某个棋牌配置下的游戏列表

    @classmethod
    @util.cached_list(lambda cls, mid: cls.ALL_IDS % ({'cid': mid}), snowslide=True)
    def _load_game_ids(cls, mid):
        games = list(cls.collection.find(
            {'chess': ObjectId(mid)},
            {'game': 1}
        ).sort("order", pymongo.ASCENDING))
...
class VipConfig(Document):
    """会员专区配置"""
    collection = DB.vip_config
    ALL_IDS = 'games:vipconfig:%(os)s'
...
class VipGames(Document):
    """会员游戏"""
    collection = DB.vip_games
    ALL_IDS = 'games:vip:%(vid)s'

    @classmethod
    @util.cached_list(lambda cls, mid: cls.ALL_IDS % ({'vid': mid}), snowslide=True)
    def _load_game_ids(cls, mid):
        games = list(cls.collection.find(
            {'vip': ObjectId(mid)},
            {'game': 1}
    ...
    @classmethod
    def vip_game_ids(cls, tpid):
        key = cls.ALL_IDS % ({'vid': tpid})
        if not Redis.exists(key):
            cls._load_game_ids(tpid)
        try:
            gids = Redis.lrange(key, 0, -1)
...
class VipGiftCode(Document):
    """会员游戏礼包"""
    collection = DB.vip_gift_codes
    ALL_IDS = 'giftcodes:vip:%(vid)s'
    ...
    def format(self):
        ...
        if uid:
            received = str(self._id) in UserGiftCodeOrder.get_user_gift_code_ids(uid)
            data.update({'received': received})
        return data

    @classmethod
    @util.cached_list(lambda cls, vid: cls.ALL_IDS % ({'vid': vid}), snowslide=True)
    def _load_giftcode_ids(cls, vid):
        _ids = list(cls.collection.find(
            {'vip': ObjectId(vid)},
            {'_id': 1}
        ).sort("order", pymongo.ASCENDING))
        gids = [str(g['_id']) for g in _ids]
        return gids
    ...
    @property
    def online(self):
        _is_online = super(VipGiftCode, self).online
        if not _is_online:
            return False
