...
class ApiTestCase(WanxTestCase):
    ...
    USER_RETURN_KEYS = [u'is_followed', u'video_count', u'user_id', u'name', u'following_count',
                        ...
                        u'status', u'bans', u'is_match', u'live_number', u'certify_status']
    ...
    def test_game(self):
        ...
        resp = self.app.get('/games/toplists?os=android&version_code=320')
        self.assertEqual(resp.status_code, 200,
                         'get game toplists error')
        rv = json.loads(resp.data)
        self.assertEqual(rv['status'], 0)
        self.assertEqual(len(rv['data']['toplists']), 1,
                         'game toplists num error!')

        toplist_id = rv['data']['toplists'][0]['id']
        resp = self.app.get('/games/toplist_games?toplist_id=%s' % toplist_id)
        self.assertEqual(resp.status_code, 200,
                         'get toplist games error')
        rv = json.loads(resp.data)
        self.assertEqual(rv['status'], 0)
        self.assertEqual(len(rv['data']['games']), 1,
                         'toplist games num error!')

        resp = self.app.get('/games/chess_configs?os=android&version_code=320')
        self.assertEqual(resp.status_code, 200,
                         'get game chess_configs error')
        rv = json.loads(resp.data)
        self.assertEqual(rv['status'], 0)
        self.assertEqual(len(rv['data']['chess_configs']), 1,
                         'game chess_configs num error!')

        chess_id = rv['data']['chess_configs'][0]['id']
        resp = self.app.get('/games/chess_games?chess_id=%s' % chess_id)
        self.assertEqual(resp.status_code, 200,
                         'get chess games error')
        rv = json.loads(resp.data)
        self.assertEqual(rv['status'], 0)
        self.assertEqual(len(rv['data']['games']), 1,
                         'chess games num error!')

        resp = self.app.get('/games/vip_configs?os=android&version_code=320')
        self.assertEqual(resp.status_code, 200,
                         'get game vip_configs error')
        rv = json.loads(resp.data)
        self.assertEqual(rv['status'], 0)
        self.assertEqual(len(rv['data']['vip_configs']), 1,
                         'game vip_configs num error!')

        vip_id = rv['data']['vip_configs'][0]['id']
        resp = self.app.get('/games/vip_games?vip_id=%s' % vip_id)
        self.assertEqual(resp.status_code, 200,
                         'get vip games error')
        rv = json.loads(resp.data)
        self.assertEqual(rv['status'], 0)
    ...
    def test_upload_large(self):
        ...
        rv = json.loads(resp.data)
        self.assertEqual(rv['data']['url'].replace("\\", "/"),
                         urljoin(app.config.get('STATIC_URL'),
    ...
    def test_gift(self):
        ...
        resp = self.app.get('/gifts/log?ut=%s&mode=1' % (self.UT))
        self.assertEqual(resp.status_code, 200, 'get gifts log error')
        rv = json.loads(resp.data)
        self.assertEqual(rv['status'], 0)
        self.assertEqual(len(rv['data']['logs']), 2)
    ...
    def test_information(self):
        _data = dict()
        resp = self.app.post('/info/electronic_sports', data=_data)
        self.assertEqual(resp.status_code, 200, 'get all electronic sports information error')
        rv = json.loads(resp.data)
        self.assertEqual(rv['status'], 0)
        self.assertEqual(len(rv['data']['info']), 2)
